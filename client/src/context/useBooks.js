...
const useBooks = () => {
	...
	useEffect(() => {
		const fetchBooks = async () => {
			try {
				...
				const wishlistBookIds = new Set(wishlistData.map((book) => book.book_id))

				// Update the `wishlist` property of the books based on the wishlist data
				const updatedBooks = data.map((book) => {
					return { ...book, wishlist: wishlistBookIds.has(book.id), isInCart: false, count: 0 }
				})
	...
	const removeFromWishlist = async (bookId) => {
		try {
			...
			setWishlistBooks((prevState) => prevState.filter((book) => book.book_id !== bookId))
		} catch (error) {
	...
	const removeFromCart = async (bookId) => {
		try {
			...
			const newBooks = books.filter((p) => bookId !== p.id)
			setBooks(newBooks)
			setFilteredBooks(newBooks)
		} catch (error) {
			console.error("Failed to delete book", error)
		...
		if (book.count > 1) {
			try {
				...
				if (response.status === 200) {
					setCartBooks((prevCartBooks) => prevCartBooks.map((b) => b.book_id === book.book_id ? { ...b, count: b.count - 1 } : b))
